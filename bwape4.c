...
void thr_bwa_print_tpx(long n_seqs)
{
	int i = 0;
...
void bwa_print_wait_tpx(void)
{
#ifdef HAVE_PTHREAD
	if( (async_read_seq) && (print_tid != 0) ){
...
void bwa_print_tpx(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs[2], const gap_opt_t opt)
{
#ifdef _TIMING
	struct timeval st;
	uint64_t s1, e1;
...
# ifdef HAVE_PTHREAD
	if( (async_read_seq) && (num_sampe_threads > 1) ){
		...
		int srtn;	
		srtn = pthread_create(&print_tid,NULL,(void *(*)(void *))thr_bwa_print_tpx,(void *)(long)n_seqs);
		if(srtn != 0){
			fprintf(stderr,"[bwa_print_tpx] pthread_create thr_bwa_print error %d\n",srtn);
			exit(1);
		}
