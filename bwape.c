...
extern void bwa_print_tpx(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs[2], const gap_opt_t opt);
extern void bwa_print_wait_tpx(void);
					   ...
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii)
{
	...
	for(i=0; i<num_sampe_threads; i++){
		d[i] = (pe_data_t*)calloc(1, sizeof(pe_data_t));
	}
...
void bwa_sai2sam_pe_core(const char *prefix, char *const fn_sa[2], char *const fn_fa[2], pe_opt_t *popt)
{
	...
	n_needed = 262144;
	if( (adj_n_needed) && (num_sampe_threads > 1) ){
		n_needed = (262144 / 2) * num_sampe_threads;
		if(n_needed > 1048576)
			n_needed = 1048576;
	}
	...
	bwa_read_seq2_tpx(ks[0], ks[1], n_needed, &n_seqs[nexti1], opt0.mode, opt.mode, 
		...
		cnt_chg = bwa_cal_pac_pos_pe(prefix, bwt, n_seqs[nexti1], seqs[nexti1], fp_sa, &ii, popt, &opt, &last_ii);
		...
		bwa_print_tpx(bns, n_seqs[nexti1], seqs[nexti1], opt);
...
int bwa_sai2sam_pe(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "a:t:o:TXsPn:N:c:f:Ar:")) >= 0) {
		switch (c) {
		case 'r':
	...
	if (optind + 5 > argc) {
		...
		fprintf(stderr, "         -X       disable async read seq method\n");
